...
import FundAccount from "../funds/buy-nebula-token.tsx";
  ...
  async function createProfile() {
    ...
    try {
      ...
      if (txhash.message === "Transaction added successfully") {
        ...
        request_payload = request_payload.filter(
          (tx: any) => tx.caller == userAccount?.address.toLowerCase()
        );
        if (request_payload.length > 0) {
            ...
            <form className="w-full">
              ...
              <Button
                className={`text-[#0f161b] uppercase font-bold tracking-[1px] px-[30px] py-3.5 border-[none] ${
                  imgUrl
                    ? "bg-[#45f882] hover:bg-[#ffbe18]"
                    : "bg-[#45f882] opacity-50 cursor-not-allowed"
