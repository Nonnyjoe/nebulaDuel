...
use crate::game_characters::{SuperPower, Character, purchase_team, get_characters, select_fighters, MinimalCharacter,confirm_ownership, get_character_details};
...
pub fn join_duel(all_duels: &mut Vec<Duel>, all_characters:&mut Vec<Character>, all_players:&mut Vec<Player>, duel_id: u128, opponent_address: String, opponent_warriors: Vec<u128>) -> Vec<Vec<u128>> {
    ...
    match selected_duel {
        Some(duel) => {
            if duel.is_active {
                ...
                let opponent_warriors = select_fighters(all_characters, all_players, opponent_address.clone(), opponent_warriors[0], opponent_warriors[1], opponent_warriors[2]);
...
pub fn set_strategy(all_duels: &mut Vec<Duel>, duel_id: u128, wallet_address: String, strategy: AllStrategies) -> &Duel {
    let selected_duel = find_duel(all_duels, duel_id);
    match selected_duel {
        Some(duel) => {
            if !duel.is_active {
...
pub fn fight(all_duels: &mut Vec<Duel>, all_characters:&mut Vec<Character>, duel_id: u128, all_players:&mut Vec<Player>, available_duels: &mut Vec<Duel>, profit_from_stake: &mut f64) -> Duel {
    ...
    match find_duel(all_duels, duel_id) {
        Some(duel) => {
            ...
            let creators_warriors: &mut Vec<Character> = &mut get_warriors_clone(all_characters, &duel.creator_warriors);
            let opponent_warriors: &mut Vec<Character> = &mut get_warriors_clone(all_characters, &duel.opponent_warriors);
            if !duel.is_active {
                panic!("duel not active");
            } else if duel.is_completed {
                return duel.clone();
            } else if duel.creators_strategy == AllStrategies::YetToSelect || duel.opponents_strategy == AllStrategies::YetToSelect {
                ...
                loop {
                    if creators_warriors.len() == 0 {
                        ...
                        register_battle_details(all_players, all_characters, duel.duel_winner.clone(), duel.duel_loser.clone(), duel.clone(), profit_from_stake);
                        ...
                        println!(".");
                        println!("NEW ROUND BEGINNING WITH OPPONENT LENGTH: {}, AND CREATOR LENTTH: {}", opponent_warriors.len(),  creators_warriors.len());
                        println!("ATTACKERS ARE: {:?}", creators_warriors);
                        println!("OPPONENT WARRIORS: {:?}", opponent_warriors);
                        println!(".");
                        println!("-----INDEX OF ATTACKER: {}", creator_turn_tracker.check_turn());
                        creator_turn_tracker.sync();
                        let mut attacker: &mut Character = creators_warriors.get_mut(creator_turn_tracker.check_turn()).unwrap();
                        let mut opponent: &mut Character = decicde_victim(creators_strategy, opponent_warriors);
                        single_duel(&mut attacker, &mut opponent);
                        duel.battle_log.push(new_vec(attacker, opponent));

                        // Check if the opponent is dead
                        if opponent.is_dead() {
                            let opponent_index = find_index_in_vec(opponent.id, opponent_warriors.clone());
                        ...
                        println!("-----INDEX OF ATTACKER2: {}", opponent_turn_tracker.check_turn());
                        attacker = opponent_warriors.get_mut(opponent_turn_tracker.check_turn()).unwrap();
                        ...
                        if opponent.is_dead() {
                            ...
                            let opponent_index = find_index_in_vec(opponent.id, creators_warriors.clone());
...
fn register_battle_details(all_players:&mut Vec<Player>, all_characters: &mut Vec<Character>, duel_winner: String, duel_looser: String, duel: Duel, profit_from_stake: &mut f64) {
...
fn get_warriors_clone(all_characters: &mut Vec<Character>, selected_characters: &Vec<u128>) -> Vec<Character> {
