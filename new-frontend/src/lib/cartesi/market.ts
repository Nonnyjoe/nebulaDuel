 ...
 * Reads via inspect (always fresh), writes via sendInput (which waits for the
 * Cartesi node to process the input before resolving).
 */
import { inspectState } from "./client";
import charactersdata from "@/lib/game/characters";
import { powerToElement, type ElementName } from "./game-types";
...
/** A single player's owned characters (decorated with element + portrait). */
export async function fetchPlayerCharacters(
  wallet: string,
): Promise<MarketCharacter[]> {
  const { ok, reports } = await inspectState(
    `players_characters/${wallet.toLowerCase()}`,
  );
  if (!ok || !reports.length) return [];
  try {
    const arr = JSON.parse(reports[0]);
