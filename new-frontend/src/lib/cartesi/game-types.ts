/**
 * Game data types + theming, mirroring app/src/campaign/mod.rs and the duel
 * battle reports. Reads level catalog / player progress / leaderboards via
 * inspect, and battle reports via campaign_battle notices.
 */
import { inspectState, listDecodedNotices } from "./client";
...
/** Latest campaign battle report for a player + level (post-fight replay source). */
...
/**
 * Battle report for a resolved P2P/AI duel. Duels carry the same campaign-format
 * report on their `battle_events` field (the unified combat engine).
 */
export async function fetchDuelBattleReport(
  duelId: number | string,
): Promise<BattleReport | null> {
  const { ok, reports } = await inspectState(`duels/${duelId}`);
  if (!ok || !reports.length) return null;
  try {
    const duel = JSON.parse(reports[0]);
    const be = duel?.battle_events;
    const report = typeof be === "string" ? JSON.parse(be) : be;
    if (report && Array.isArray(report.events) && report.events.length) {
      return report as BattleReport;
    }
    return null;
  } catch {
    return null;
...
/** Element → 3D model pool for campaign enemies (paths under /public/models). */
